...
Author: Huahai
Category: opinion
Tags: AI,Programming Languages,Static Typing,Software Development,LLM
---
...
That argument rests on an assumption that is rapidly becoming outdated:

The person writing the code is human.

AI is not human.

It does not prefer Python because Python feels simple. It does not admire Rust
because Rust feels rigorous. It has no taste, no emotional attachment, and no
...
## AI Does Not Need a Type System to Teach It How to Code
...
A type checker cannot save you from that.

The claim that AI needs "more compiler feedback" sounds technical, but it often
amounts to repeating an old argument without thinking about what AI actually
needs.

AI generally does not need the compiler to explain that a string is not an
integer.

It needs a more accurate understanding of reality.

## More Language Means More Tokens
...
In one language, a simple operation may require:
...
Another language may express the same behavior in fifteen lines.

Those additional structures are not free.

Longer code requires more generated tokens. More symbols must remain consistent
across the context. Changes touch more declarations and more files. Every
additional abstraction creates another place where the model can misunderstand
the programmer's intent.

AI does not automatically become more correct merely because the code looks
more rigorous.

It simply has more things to keep consistent.
...
into the compilation stage.

In AI-assisted development, compilation errors are among the cheapest errors
possible.

They are explicit, deterministic, easy to locate, and usually trivial to repair
automatically.
...
of rules represented by its type system.

AI-generated code already spends very little time stuck on basic compilation
...
It is not entirely useless. It is simply no longer the main issue.

## Language Costs Must Be Recalculated for the AI Era

This does not mean types have no value.

Types can document interfaces, define module boundaries, support tooling, and
encode genuine domain constraints. Systems with demanding performance, safety,
or verification requirements may still justify the cost of a sophisticated
type system.
...
culture war between static and dynamic typing.

It is generating tokens.
...
which humans were the primary producers of code.

That premise has changed.

The conclusion should change with it.
